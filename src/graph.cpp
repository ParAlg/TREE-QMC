...
Graph::Graph(std::vector<Tree *> trees, Taxa &subset, std::string weighting) {
  ...
  graph = parlay::reduce(
      subgraphs, parlay::binary_op(
                     [&](weight_t ***g1, weight_t ***g2) -> weight_t *** {
  ...
  */
  if (verbose > "1")
    subproblem_csv << ',' << count[1] << ',' << count[2] << ',' << count[3];
  t.stop();
  t.total();
}
...
Graph::Graph(std::unordered_map<quartet_t, weight_t> &quartets, Taxa &subset) {
  ...
  for (auto elem : quartets) {
    ...
    index_t a = index2index[indices[0]], b = index2index[indices[1]],
            c = index2index[indices[2]], d = index2index[indices[3]];
...
weight_t Graph::sdp_cut(weight_t alpha, std::vector<index_t> *A,
                        std::vector<index_t> *B, unsigned long int iter_limit) {
  ...
  for (index_t i = 0; i < size; i++) {
    for (index_t j = i + 1; j < size; j++) {
      ...
      input.push_back(
          Instance::InstanceTuple(std::make_pair(i + 1, j + 1), weight));
    }
  }
...
bool QuartetGraphMaxCutCallback::Report(const MaxCutSimpleSolution &solution,
